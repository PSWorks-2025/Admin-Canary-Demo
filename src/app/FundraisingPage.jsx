...
const FundraisingPage = () => {
  ...
  useEffect(() => {
    ...
    console.log("Totals calculated:", { amountRaised, totalCollected, totalSpent });
  ...
  const handleTransactionChange = useCallback(async (index, field, value) => {
  ...
  const addTransaction = useCallback(async () => {
    const newId = localData.transactions.length > 0
      ? Math.max(...localData.transactions.map(tx => tx.id)) + 1
      : 1;
    const today = new Date().toISOString().split("T")[0];
    const newTransaction = { id: newId, type: "Thu", amount: 0, date: today };
    setLocalData(prev => ({ ...prev, transactions: [...prev.transactions, newTransaction] }));
    await updateMainData({
      fundraising: {
        ...mainData.fundraising,
        transactions: [...(mainData.fundraising?.transactions || []), { ...newTransaction, date: Timestamp.fromDate(new Date(today)) }],
      },
    });
    console.log("New transaction added:", newTransaction);
  }, [localData.transactions, mainData.fundraising]);

  const deleteTransaction = useCallback(async (index) => {
    console.log(`Deleting transaction at index ${index}`);
    const newTransactions = localData.transactions.filter((_, i) => i !== index);
    setLocalData(prev => ({ ...prev, transactions: newTransactions }));
    await updateMainData({
      fundraising: {
        ...mainData.fundraising,
